...
def extract_metadata(mp3_path):
    ...
    try:
        ...
        tags = audio.tags
        meta = {}
        meta['TIT2'] = clean_string(tags.get('TIT2', 'Unknown Title')) if tags else 'Unknown Title'
        meta['TPE1'] = clean_string(tags.get('TPE1', 'Unknown Artist')) if tags else 'Unknown Artist'
        meta['TALB'] = clean_string(tags.get('TALB', 'Unknown Album')) if tags else 'Unknown Album'
        if tags:
    ...
    while True:
        ...
        for file in files:
            ...
            def extract_and_store():
                ...
                if meta is not None:
                    meta_path = os.path.join(META_DIR, os.path.splitext(file)[0] + '.mp3.json')
            ...
            try:
                ...
                notify_all("Karaoke Pipeline Error", f"❌ Metadata extraction failed for {file}: {e}")
